"""Consume proven separately pushed return segments in native stack tracking.

Layer: Frontend/angr compatibility.
Responsibility: apply authoritative Semantics return-frame and cleanup proofs after
angr accounts for the near CALL's own return address. Do not infer far returns
...
def _cleanup_target_8616(project: Project, vex: pyvex.IRSB) -> int | None:
    ...
    return resolve_direct_call_target_from_block(project, vex.addr)
