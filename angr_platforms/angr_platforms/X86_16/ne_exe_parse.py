...
def parse_ne_header(data: bytes, ne_offset: int) -> tuple[dict[str, int | str], int]:
    ...
    try:
        ...
        header["entry_table_off"], header["entry_table_len"] = struct.unpack_from("<HH", data, ne_offset + 0x04)
        ...
        header["entry_cs"], header["entry_ip"] = struct.unpack_from("<HH", data, ne_offset + 0x14)
        header["entry_ss"], header["entry_sp"] = struct.unpack_from("<HH", data, ne_offset + 0x18)
...
def parse_ne_segment_table(data: bytes, ne_offset: int, seg_table_off: int, seg_count: int) -> list[NESegment]:
    ...
    try:
        for _ in range(seg_count):
            ...
            seg_offset, seg_length, seg_flags, min_alloc = struct.unpack_from("<HHHH", data, pos)
            pos += 8

            segments.append(NESegment(offset=seg_offset, length=seg_length, flags=seg_flags, min_alloc=min_alloc))
